...
- Adds a Developer system-prompt editor covering Turp's core/runtime/tool/research/memory/file/execution/generated-content/Sudo/continuation/provider-guard/auxiliary components. It uses a searchable grouped prompt manager, edits concrete prompt text directly, preserves saved edits when components are disabled, supports per-component reset and confirmed reset-all, shows last rendered built-in text for dynamic components, and exposes the final ordered system context in a dedicated inspector.
