...
- Turp'un çekirdek/çalışma zamanı/araç/araştırma/hafıza/dosya/yürütme/üretilen içerik/Sudo/devam/sağlayıcı koruması/yardımcı model bileşenlerini kapsayan Geliştirici sistem istemi düzenleyicisini ekler. Aranabilir ve gruplandırılmış bir istem yöneticisi kullanır; somut istem metni doğrudan düzenlenebilir, bileşen devre dışı bırakıldığında kayıtlı düzenleme korunur, bileşen bazında sıfırlama ve onaylı tümünü sıfırlama desteklenir; dinamik bileşenlerin son oluşturulan yerleşik metni ve son sıralı sistem bağlamı ayrı bir denetleyicide görüntülenebilir.
