...
import app.turp.chat.MainActivity
import app.turp.chat.R
import app.turp.chat.installedAppVersion
import app.turp.chat.agent.TurpNativeTools
...
import app.turp.chat.chat.ContextAssembler
import app.turp.chat.chat.CostCalculator
...
import app.turp.chat.provider.NativeToolResult
import app.turp.chat.provider.ProviderCredentialPolicy
import app.turp.chat.provider.ProviderHttpException
...
class GenerationWorker(
    ...
    private val container = (appContext.applicationContext as TurpApplication).container
    ...
    private suspend fun generate() {
        ...
        suspend fun executeTool(
            ...
            val input = if (normalizedTool in setOf("compile_widget", "widget_compile")) {
                "turp-widget candidate • ${request.source?.length ?: 0} characters"
            } else {
            ...
            // identical tool calls retain Turp's side-effect replay guard.
        ...
        fun roughInputTokens(inputs: List<InputMessage>): Int = inputs.sumOf { input ->
            app.turp.chat.chat.TokenEstimator.estimate(input.content + input.reasoning + input.toolTraceJson) +
                input.attachments.sumOf { attachment ->
                    when {
                        attachment.extractedText != null -> app.turp.chat.chat.TokenEstimator.estimate(attachment.extractedText.take(1_000_000)) + 64
                        attachment.ocrJson != null -> app.turp.chat.chat.TokenEstimator.estimate(attachment.ocrJson.take(128_000)) + 64
        ...
        for (round in 0..(maxToolRounds + 1)) {
            ...
            suspend fun upsertToolCallProgress(progress: app.turp.chat.provider.NativeToolCallProgress, requestId: String) {
                closeOpenStreamEvents()
    ...
    private fun notificationBuilder(text: String, indeterminate: Boolean): NotificationCompat.Builder {
        ...
        return NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_turp_monochrome)
    ...
    companion object {
        ...
        const val CHANNEL_ID = "turp_generation"
        ...
        private const val INITIAL_RESEARCH_STATE_INSTRUCTION =
            "Deep Research is active. Before doing any research, output ONLY one <turp-research-state> XML-wrapped JSON block. " +
        ...
        private const val UPDATE_RESEARCH_STATE_INSTRUCTION =
            "Output ONLY one updated <turp-research-state> XML-wrapped JSON block based on the roadmap and latest tool result already present. " +
        ...
        private const val FINAL_RESEARCH_STATE_INSTRUCTION =
            "Output ONLY one final <turp-research-state> XML-wrapped JSON block for the research response you just produced. " +
        ...
        private const val RESEARCH_STATE_CONTINUATION_REMINDER =
            "\n\nMANDATORY DEEP RESEARCH PROTOCOL: Before your next tool call or user-facing prose, emit one updated <turp-research-state> block in normal response text. " +
